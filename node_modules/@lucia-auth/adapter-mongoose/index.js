...
const createMongoValues = (object) => {
    ...
    return (LuciaError) => {
        return {
            ...
            getSessionAndUserBySessionId: async (sessionId) => {
                ...
                if (!session)
                    return null;
                const user = await User.findById(session.user_id, DEFAULT_PROJECTION).lean();
                if (!user)
                    return null;
                return {
            ...
            setUser: async (userId, userAttributes, key) => {
                ...
                const userDoc = new User(createMongoValues({
                    id: userId,
                    ...userAttributes
                }));
                await userDoc.save();
            ...
            setSession: async (session) => {
                const userDoc = await User.findById(session.user_id, DEFAULT_PROJECTION).lean();
                if (!userDoc)

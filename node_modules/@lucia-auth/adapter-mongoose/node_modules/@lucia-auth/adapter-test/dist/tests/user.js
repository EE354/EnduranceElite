...
import { isNull, isEmptyArray, expectErrorMessage, expectError } from "../validate.js";
export const testUserAdapter = async (adapter, queryHandler, endProcess = true) => {
    ...
    await test("setUser()", "Insert user", async () => {
        const user = database.user();
        await adapter.setUser(user.value.id, { username: user.value.username }, null);
        await user.exists();
        await clearAll();
    });
    await test("setUser()", "Insert user - Return the created user", async () => {
        const user = database.user();
        const result = await adapter.setUser(user.value.id, { username: user.value.username }, null);
        user.compare(result);
        await clearAll();
    });
    await test("setUser()", "Insert user and persistent key", async () => {
        ...
        await adapter.setUser(user.value.id, { username: user.value.username }, key.value);
        await user.exists();
        await clearAll();
    });
    await test("setUser()", "Insert user and persistent key - Return created user", async () => {
        const user = database.user();
        const key = user.key({
            primary: true,
            passwordDefined: true,
        ...
        const result = await adapter.setUser(user.value.id, { username: user.value.username }, key.value);
        user.compare(result);
        await clearAll();
    });
    await test("setUser()", "Throw AUTH_DUPLICATE_KEY_ID if key already exists", async () => {
    ...
    await test("updateUserAttributes()", "Returns updated user or void", async () => {
        const user = database.user();
        await user.commit();
        user.update({
            username: "UPDATED"
    ...
    await test("updateUserAttributes()", "Throw INVALID_USER_ID or return void if user id is invalid", async () => {
        const user = database.user();
        await user.commit();
    ...
    await test("setKey()", "Insert a new persistent key with null password", async () => {
        const user = database.user();
        await user.commit();
        const key = user.key({
            primary: false,
    ...
    await test("setKey()", "Insert a new single use key with null password", async () => {
        const user = database.user();
        await user.commit();
        const key = user.key({
            primary: false,
    ...
    await test("setKey()", "Throw AUTH_INVALID_USER_ID if user id is invalid", async () => {
        const key = database.user().key({
            primary: false,
            passwordDefined: true,
            oneTime: false
    ...
    await test("setKey()", "Throw AUTH_DUPLICATE_KEY_ID if key already exists", async () => {
        const key = database.user().key({
            primary: false,
            passwordDefined: true,
            oneTime: false
    ...
    await test("getKeysByUserId()", "Returns an empty array if no sessions exist", async () => {
    ...
    await test("updateKeyPassword()", "Throw AUTH_INVALID_KEY_ID if key id is invalid", async () => {
        const key = database.user().key({
            primary: false,
            passwordDefined: true,
            oneTime: false
        ...
        const returnedKey = await adapter.updateKeyPassword(key.value.id, key.value.hashed_password);
        if (returnedKey !== undefined) {
            key.compare(returnedKey);
        }
        await clearAll();
    });
    await test("updateKeyPassword()", "Throw AUTH_INVALID_KEY_ID or return void if key id is invalid", async () => {
        const key = database.user().key({
            primary: false,
            passwordDefined: true,
            oneTime: false
        ...
        await key.commit();
        expectErrorMessage(async () => {
            const returnedKey = await adapter.updateKeyPassword(INVALID_INPUT, key.value.hashed_password);
            if (returnedKey === undefined) {

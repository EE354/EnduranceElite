...
export const testAdapter = async (adapter, queryHandler, endProcess = true) => {
...
const testOptionalMethods = async (adapter, queryHandler, endProcess = true) => {
    const database = new Database(queryHandler);
    const clearAll = database.clear;
    await test("getSessionAndUserBySessionId()", "Return the correct user and session", async () => {
        if (!adapter.getSessionAndUserBySessionId)
            return;
        const user = database.user();
        const session = user.session();
        await session.commit(); // this will set user as well
        const result = await adapter.getSessionAndUserBySessionId(session.value.id);

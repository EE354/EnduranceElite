...
export const testSessionAdapter = async (adapter, queryHandler, endProcess = true) => {
    ...
    await test("setSession()", "Throw AUTH_DUPLICATE_SESSION_ID if session id is already in use", async () => {
        const session1 = database.user().session();
        await session1.commit();
        const user = database.user();
        await user.commit();
